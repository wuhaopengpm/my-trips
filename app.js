...
const {escapeHTML,safeBackgroundPosition,normalizeTripPack,pictureHTML}=MyTripsUtils;
...
function temporal(day){
  ...
  if(!day.timeline.length)return{mode:'empty'};
...
function renderHeader(){
  ...
  $('#statusPill').innerHTML=`<span class="dot" style="${navigator.onLine?'':'background:#a64035'}"></span>${navigator.onLine?'网络可用':'离线可用'}`;
}
...
async function openTripByRef(ref){
  try{
    TRIP=normalizeTripPack(await fetch(ref.data).then(r=>{if(!r.ok)throw new Error('load');return r.json()}));
...
function openImportedTrip(id){
  ...
  TRIP=normalizeTripPack(JSON.parse(raw));state.day=defaultDay();state.view='today';localStorage.setItem('lastTripId',TRIP.id);render();
}
function importedTrips(){
  const arr=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith('imported:')){try{const t=normalizeTripPack(JSON.parse(localStorage.getItem(k)));arr.push(t)}catch(e){}}}
  return arr;
}
...
function tripCardHTML(t){
  ...
  const cl=checklistCount(t),cover=t.coverImage||'',media=cover?pictureHTML({src:cover,srcset:t.coverImageSrcset,sizes:t.coverImageSizes,alt:`${t.city}旅行封面`,priority:true,className:'cover-media'}):'';
  const coverStyle=`style="--cover-position:${safeBackgroundPosition(t.coverPosition)}"`;
  return `<section class="card trip-card">
    <div class="trip-cover ${escapeHTML(theme)} ${cover?'has-photo':''}" ${coverStyle}>${media}<div class="cover-overlay"></div>
...
function renderLibrary(){
  ...
  $('#tripImport').addEventListener('change',async e=>{
    ...
    try{
      const t=normalizeTripPack(JSON.parse(await f.text()));
...
function scenicHero(day){
  ...
  const media=pictureHTML({src:TRIP.coverImage,srcset:TRIP.coverImageSrcset,sizes:TRIP.coverImageSizes,alt:`${TRIP.city} · ${day.title}`,priority:true,className:'cover-media'});
  return `<section class="scenic-hero" style="--cover-position:${safeBackgroundPosition(TRIP.coverPosition)}">${media}<div class="cover-overlay"></div>
...
function smartCard(day){
  ...
  else if(t.mode==='empty'){main=`今天 Day ${day.day}`;sub='当天暂未安排时间轴。';rLabel='住宿';r=day.hotel}
  ...
  return `<section class="card smart"><div class="smart-label">${escapeHTML(TRIP.city)} SMART TRIP</div><div class="smart-main">${escapeHTML(main)}</div><div class="smart-sub">${escapeHTML(sub)}</div>
...
function tomorrowCard(){
  ...
  return `<section class="card tomorrow"><div class="section-head" style="margin:0 0 6px"><h3 style="font-size:16px">明天提前准备</h3><span class="hint">Day ${d.day} · ${d.label}</span></div>
    <div class="route">${escapeHTML(d.title)}<br><strong>住宿：</strong>${escapeHTML(d.hotel)}<br><strong>预订：</strong>${escapeHTML(d.booking)}</div>
    <div class="tomorrow-grid">${items.map(x=>`<div class="tomorrow-item"><b>${escapeHTML(x[0])} ${escapeHTML(x[1])}</b><span>${escapeHTML(x[2])}</span></div>`).join('')}</div></section>`;
}
function goToday(){
...
function renderToday(){
  ...
  $('#main').innerHTML=`<div class="quickrow"><button class="btn soft" id="goLibrary">← 旅行库</button><button class="btn ${state.day===realDay?'primary':'soft'}" id="goToday">◎ 回到今天</button><button class="btn soft" id="goOverview">整趟路线</button><button class="btn soft" id="goChecklist">✓ Checklist</button></div>`+
    scenicHero(d)+smartCard(d)+`<section class="card day-detail-card">
    <div class="day-detail-title"><span>DAY ${escapeHTML(d.day)} DETAILS</span><b>${escapeHTML(d.hotel)}</b></div>
    <div class="meta-grid"><div class="meta"><b>交通安排</b><span>${escapeHTML(d.transport)}</span></div><div class="meta"><b>预订重点</b><span>${escapeHTML(d.booking)}</span></div><div class="meta"><b>今日时间点</b><span>${d.timeline.length} 个</span></div><div class="meta"><b>行程日期</b><span>${escapeHTML(d.label)}</span></div></div></section>
    <div class="notice"><b>备用方案</b>${escapeHTML(d.backup)}</div>${tomorrowCard()}
  ...
  $('#goLibrary').addEventListener('click',()=>setView('library'));$('#goToday').addEventListener('click',goToday);$('#goOverview').addEventListener('click',()=>setView('overview'));$('#goChecklist').addEventListener('click',()=>setView('tools'));
  let current=-1;if(['live','day-done'].includes(t.mode))current=t.current;
  if(!d.timeline.length)$('#timeline').innerHTML='<div class="route empty-state">当天暂无时间轴。</div>';
  d.timeline.forEach((x,i)=>{const key=`${d.date}-${i}`,done=!!checks[key],row=document.createElement('div');row.className='item'+(i===current?' current':'');row.innerHTML=`<div class="time">${escapeHTML(x[0])}</div><button class="check ${done?'done':''}" aria-label="${done?'取消完成':'标记完成'}：${escapeHTML(x[1])}" aria-pressed="${done}"></button><div class="item-body ${done?'done':''}">${i===current?'<div class="current-tag">现在 / 最近一项</div>':''}<h4>${escapeHTML(x[1])}</h4><p>${escapeHTML(x[2])}</p></div>`;row.querySelector('.check').addEventListener('click',()=>{checks[key]=!checks[key];saveChecks(checks);renderToday()});$('#timeline').appendChild(row)});
  if(!d.places.length)$('#places').innerHTML='<div class="route empty-state">当天暂无地点坐标。</div>';
  d.places.forEach(([name,lat,lng])=>{const div=document.createElement('div');div.className='place';div.innerHTML=`<div><div class="place-name">${escapeHTML(name)}</div><div class="coords">${escapeHTML(lat)}, ${escapeHTML(lng)}</div></div><div class="actions"><a class="btn" href="https://maps.apple.com/?q=${encodeURIComponent(name)}&ll=${lat},${lng}" target="_blank" rel="noopener">Apple 地图</a><a class="btn" href="https://www.google.com/maps/search/?api=1&query=${lat},${lng}" target="_blank" rel="noopener">Google Maps</a></div>`;$('#places').appendChild(div)})
}
function renderOverview(){
  renderDayStrip(false);$('#main').innerHTML=`<section class="card hero"><span class="pill">${escapeHTML(TRIP.city)}</span><h2>整趟路线</h2><div class="route">${escapeHTML(TRIP.meta.route)}<br>${escapeHTML(TRIP.meta.start)} → ${escapeHTML(TRIP.meta.end)}</div></section><section class="card" id="overview"></section>
    <div class="section-head"><h3>酒店候选</h3><span class="hint">当前攻略包</span></div><section class="card" id="hotels"></section>`;
  TRIP.days.forEach((d,i)=>{const row=document.createElement('button');row.className='overview-day';row.innerHTML=`<div class="daynum">D${escapeHTML(d.day)}</div><div><h4>${escapeHTML(d.label)} · ${escapeHTML(d.title)}</h4><p>${escapeHTML(d.route)}</p></div><div class="chev">${completedCount(d)}/${d.timeline.length} ›</div>`;row.addEventListener('click',()=>{state.day=i;setView('today')});$('#overview').appendChild(row)});
  $('#hotels').innerHTML=(TRIP.hotels||[]).map(a=>`<div style="margin-bottom:16px"><div class="section-head" style="margin:0 0 6px"><h3 style="font-size:15px">${escapeHTML(a.area)}</h3><span class="hint">${escapeHTML(a.night)}</span></div>${(a.choices||[]).map(h=>`<div style="padding:11px 0;border-top:1px solid var(--line)"><b style="font-size:14px">${escapeHTML(h.name)}</b><span class="pill" style="font-size:10px;padding:4px 7px;margin-left:6px">${escapeHTML(h.tag)}</span><p class="route" style="font-size:12px;margin:5px 0 0">${escapeHTML(h.why)}</p></div>`).join('')}</div>`).join('')||'<div class="route">此攻略包暂无酒店候选。</div>';
}
...
async function compressOrderImage(file){
  if(file.size>6*1024*1024)throw new Error('请选择 6MB 以下截图');
  const url=URL.createObjectURL(file);
  try{
    const img=await new Promise((resolve,reject)=>{const value=new Image();value.onload=()=>resolve(value);value.onerror=reject;value.src=url});
...
function closeOverlay(element){element.classList.remove('open');overlayReturnFocus?.focus?.();overlayReturnFocus=null}
function openViewer(title,image){
  overlayReturnFocus=document.activeElement;$('#viewerTitle').textContent=title;$('#viewerImg').src=image;$('#imageViewer').classList.add('open');$('#viewerClose').focus();
}
async function renderOrders(){
  renderDayStrip(false);const tripId=TRIP.id,all=await getOrders().catch(()=>[]);if(state.view!=='orders'||TRIP?.id!==tripId)return;const saved=all.filter(x=>x.tripId===TRIP.id),map=new Map(saved.map(x=>[x.orderId,x]));
  const templates=TRIP.orderTemplates||[],types=['全部',...new Set(templates.map(x=>x.type))];
  $('#main').innerHTML=`<section class="card hero"><span class="pill">${escapeHTML(TRIP.city)} · OFFLINE WALLET</span><h2>订单与二维码</h2><div class="route">点订单截图可全屏放大；重要订单可置顶。</div></section><div class="order-filter">${types.map(t=>`<button class="filter ${t===state.orderFilter?'active':''}" data-type="${escapeHTML(t)}">${escapeHTML(t)}</button>`).join('')}</div><div id="orders"></div>`;
  ...
  list.forEach(t=>{const s=map.get(t.id),c=document.createElement('section');c.className='card order-card'+(s?.pinned?' pinned':'');c.innerHTML=`<div class="order-head"><div><div class="order-type">${escapeHTML(t.type)}${s?.pinned?' · ★ 重要':''}</div><h4>${escapeHTML(t.title)}</h4><div class="order-status">${s?'已保存到本机':'尚未添加'}</div></div><button class="btn edit">${s?'编辑':'添加'}</button></div>${s?.image?`<div class="order-preview" title="点按全屏"><img src="${escapeHTML(s.image)}" alt="${escapeHTML(t.title)}" loading="lazy"></div>`:''}${s?.note?`<div class="order-note">${escapeHTML(s.note)}</div>`:''}${s?`<div class="actions" style="margin-top:10px"><button class="btn soft pin">${s.pinned?'取消置顶':'★ 设为重要'}</button><button class="btn danger del">删除本机副本</button></div>`:''}`;
...
function openOrderSheet(t,s){
  overlayReturnFocus=document.activeElement;$('#orderModal').classList.add('open');$('#orderSheetTitle').textContent=t.title;$('#orderNote').value=s?.note||'';$('#orderFile').value='';$('#orderCurrent').innerHTML=s?.image?`<div class="order-preview" id="sheetPreview"><img src="${escapeHTML(s.image)}"></div>`:'<div class="hint">未保存图片</div>';$('#orderClose').focus();
  const sp=$('#sheetPreview');if(sp)sp.addEventListener('click',()=>openViewer(t.title,s.image));
  $('#orderSave').onclick=async()=>{const f=$('#orderFile').files[0];let image=s?.image||'';try{if(f)image=await compressOrderImage(f);await putOrder({key:`${TRIP.id}:${t.id}`,tripId:TRIP.id,orderId:t.id,image,note:$('#orderNote').value.trim(),pinned:s?.pinned||false,updatedAt:Date.now()});closeOverlay($('#orderModal'));renderOrders()}catch(error){alert(error.message||'图片保存失败')}}
}
...
function renderChecklist(){
  ...
  return `<div class="section-head"><h3>行前 Checklist</h3><span class="hint">本机自动保存</span></div>
    ...
    ${groups.map(g=>`<div class="checkgroup"><div class="checkgroup-title">${escapeHTML(g)}</div>${list.filter(x=>(x.group||'其他')===g).map(x=>`<div class="checkrow ${doneMap[x.id]?'done':''}"><button class="check ${doneMap[x.id]?'done':''}" data-checkid="${escapeHTML(x.id)}" aria-label="${doneMap[x.id]?'取消完成':'标记完成'}：${escapeHTML(x.title)}" aria-pressed="${!!doneMap[x.id]}"></button><div><h4>${escapeHTML(x.title)}</h4><p>${escapeHTML(x.note||'')}</p></div></div>`).join('')}</div>`).join('')||'<div class="route">这个攻略包暂时没有 Checklist。</div>'}
    </section>`;
}
function renderTools(){
  ...
  ${Object.entries(guides).map(([k,v])=>`<div class="section-head"><h3>${escapeHTML(k)}</h3></div><section class="card"><ul class="guide-list">${(Array.isArray(v)?v:[]).map(x=>`<li>${escapeHTML(x)}</li>`).join('')}</ul></section>`).join('')}
  <section class="card"><div class="actions"><button class="btn" id="exportAll">导出本机数据</button><button class="btn wide" id="backLibrary">← 返回旅行库</button></div></section>`;
  ...
  $$('#main .contact-row input').forEach(inp=>inp.addEventListener('change',()=>{const c=loadContacts();c[inp.dataset.key]=inp.value.trim();saveContacts(c)}));$('#exportAll').addEventListener('click',exportAllLocalData);$('#backLibrary').addEventListener('click',()=>setView('library'))
}
async function exportAllLocalData(){
  const storage={};for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i);if(/^(checks:|checklist:|contacts:|finance:|imported:)/.test(key))storage[key]=localStorage.getItem(key)}
  const payload={version:'3.7.0',exportedAt:new Date().toISOString(),localStorage:storage,orders:await getOrders().catch(()=>[])};
  const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='my-trips-backup.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}
function render(){
...
async function init(){
  ...
  $('#orderClose').addEventListener('click',()=>closeOverlay($('#orderModal')));
  $('#orderModal').addEventListener('click',e=>{if(e.target.id==='orderModal')closeOverlay(e.currentTarget)});
  $('#viewerClose').addEventListener('click',()=>closeOverlay($('#imageViewer')));
  $('#imageViewer').addEventListener('click',e=>{if(e.target.id==='imageViewer')closeOverlay(e.currentTarget)});
  window.addEventListener('keydown',e=>{if(e.key==='Escape'){if($('#imageViewer').classList.contains('open'))closeOverlay($('#imageViewer'));else if($('#orderModal').classList.contains('open'))closeOverlay($('#orderModal'))}});
  ...
  if(active){
    ...
    else{const ref=INDEX.trips.find(x=>x.id===active.id);try{TRIP=normalizeTripPack(await fetch(ref.data).then(r=>r.json()))}catch(e){}}
