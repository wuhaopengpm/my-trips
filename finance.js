...
(function(){
  ...
  window.renderFinance=function(){
    ...
    $('#main').innerHTML=`
      ...
      <div class="section-head"><h3>记一笔</h3><span class="hint">IDR / CNY 都可以</span></div><section class="card"><div class="expense-form"><div class="field"><label>金额</label><input id="expenseAmount" type="number" min="0" step="1" placeholder="金额"></div><div class="field"><label>币种</label><select id="expenseCurrency"><option>IDR</option><option>CNY</option></select></div><div class="field"><label>分类</label><select id="expenseCategory">${cats.map(c=>`<option>${c}</option>`).join('')}</select></div><div class="field"><label>日期</label><input id="expenseDate" type="date" value="${localParts().date}"></div><div class="field"><label>项目</label><input id="expenseTitle" type="text" placeholder="例如 Grab / 晚餐"></div><div class="field"><label>备注</label><input id="expenseNote" type="text" placeholder="可选"></div><div class="wide-row"><button class="btn primary wide" id="addExpense">添加消费</button></div></div></section>
      <div class="section-head"><h3>分类统计</h3><span class="hint">统一按 IDR</span></div><section class="card">${cats.map(c=>`<div class="cat-row"><div class="cat-name">${c}</div><div class="cat-bar"><i style="width:${Math.round((catTotals[c]||0)/maxCat*100)}%"></i></div><div class="cat-value">${money(catTotals[c]||0)}</div></div>`).join('')}</section>
